...
import {
  FaCheck, FaClock, FaCopy, FaExternalLinkAlt, FaFileAlt, FaFolder, FaLink, FaLock, FaShareAlt, FaTrashAlt,
} from "react-icons/fa";
...
/**
 * Shows all links created by the user (share management page).
 * @returns {JSX.Element}
 */
export function SharedByMePage() {
  ...
  const downloadShares = activeShares.filter((share) => share.permission === "download");
  ...
  async function handleRevoke(shareId) {
    try {
      ...
      setShares((currentShares) => currentShares.map((share) => share.id === shareId ? { ...share, revokedAt: new Date() } : share));
  ...
  function handleOpenTarget(share) {
    ...
    if (share.targetType === "directory") { navigate(targetPath); return; }
    window.location.href = targetPath;
  }

  const filters = [["active", "Live"], ["expired", "Expired"], ["revoked", "Revoked"], ["all", "All"]];
      ...
      <div className="flex min-h-screen bg-gradient-to-br from-slate-50 to-slate-100">
        ...
        <main className="flex-1 min-h-screen px-4 md:px-6 py-6 pb-24 md:pb-10 md:pl-60">
          <div className="mx-auto max-w-6xl">
              ...
              <div className="relative px-6 py-7 text-white sm:px-8">
                ...
                <div className="relative flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
                  <div>
                    ...
                    <h1 className="text-2xl md:text-3xl font-black tracking-tight">Shared by me</h1>
                    <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-300">
                      Review every link you have created, copy live links, open the original item, or revoke access immediately.
                    </p>
                  </div>

                  <div className="grid grid-cols-3 gap-3 text-center">
                    <div className="rounded-2xl border border-white/10 bg-white/10 px-4 py-3">
                      <p className="text-xl md:text-2xl font-black">{activeShares.length}</p>
                      <p className="text-[11px] font-bold uppercase tracking-[0.08em] text-slate-400">Live</p>
                    </div>
                    <div className="rounded-2xl border border-white/10 bg-white/10 px-4 py-3">
                      <p className="text-xl md:text-2xl font-black">{downloadShares.length}</p>
                      <p className="text-[11px] font-bold uppercase tracking-[0.08em] text-slate-400">Download</p>
                    </div>
                    <div className="rounded-2xl border border-white/10 bg-white/10 px-4 py-3">
                      <p className="text-xl md:text-2xl font-black">{shares.length}</p>
                      <p className="text-[11px] font-bold uppercase tracking-[0.08em] text-slate-400">Total</p>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div className="mb-5 flex flex-col gap-3 rounded-2xl border border-slate-200/60 bg-white/80 p-3 shadow-sm sm:flex-row sm:items-center">
                ...
                {filters.map(([value, label]) => (
                  <button
                    ...
                    className={`min-h-9 flex-1 rounded-lg px-3 text-xs font-bold transition-all ${
                      filter === value ? "bg-white text-slate-950 shadow-sm" : "text-slate-500 hover:text-slate-800"
                    }`}
                  >
                    {label}
                  </button>
            ...
            {loading ? (
              <div className="rounded-3xl border border-slate-200/60 bg-white/80 p-10 text-center text-sm font-semibold text-slate-500">
              ...
              <div className="grid gap-4">
                {filteredShares.map((share) => {
                  ...
                  return (
                    <article
                      key={share.id}
                      className="group rounded-3xl border border-slate-200/60 bg-white/90 p-4 shadow-sm transition-all hover:-translate-y-0.5 hover:shadow-md"
                    >
                        ...
                        <div className="flex min-w-0 flex-1 items-center gap-4">
                          ...
                          <div className="min-w-0">
                            <div className="flex flex-wrap items-center gap-2">
                              <h2 className="truncate text-base font-bold text-slate-950">{share.target?.name || "Deleted item"}</h2>
                              <span className={`rounded-full px-2.5 py-1 text-[10px] font-bold uppercase tracking-[0.08em] ${isActive ? "bg-emerald-50 text-emerald-700" : "bg-slate-100 text-slate-500"}`}>
                                {getStatusLabel(status)}
                              </span>
                            </div>
                            <div className="mt-1 flex flex-wrap gap-x-4 gap-y-1 text-xs font-medium text-slate-500">
                              <span className="capitalize">{share.targetType}</span>
                              <span>{formatSize(share.target?.size || 0)}</span>
                              <span className="capitalize">{share.permission === "download" ? "View and download" : "View only"}</span>
              ...
              <div className="rounded-3xl border border-slate-200/60 bg-white/90 px-8 py-16 text-center shadow-sm">
                <div className="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-3xl bg-cyan-50 text-cyan-600">
                  <FaShareAlt />
                </div>
                <h2 className="text-lg font-bold text-slate-950">No shared links here yet</h2>
                <p className="mx-auto mt-2 max-w-md text-sm leading-6 text-slate-500">
                  Open any file or folder menu, choose Share, and created links will appear here for quick access.
                </p>
                <button
                  type="button"
                  onClick={() => navigate("/")}
                  className="mt-6 rounded-xl bg-gradient-to-r from-blue-500 to-indigo-600 px-5 py-3 text-sm font-bold text-white shadow-md transition-all hover:shadow-lg"
                >
                  Go to My Drive
                </button>
              </div>
